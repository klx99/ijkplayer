...
static void video_sync_callback(void *opaque); // JsView Added
...
IjkMediaPlayer_native_setup(JNIEnv *env, jobject thiz, jobject weak_this)
{
    ...
    ijkmp_set_video_sync_callback(mp, video_sync_callback, ijkmp_get_weak_thiz(mp));
...
// JsView Added >>>
static void video_sync_callback(void *opaque)
{
    JNIEnv *env = NULL;
    jobject weak_this = (jobject) opaque;
    ...
    J4AC_IjkMediaPlayer__onVideoSync(env, weak_this);
    if (J4A_ExceptionCheck__catchAll(env)) {
        ALOGE("%s: onVideoSync failed\n", __func__);
        return;
    }
...
static jint
IjkMediaPlayer_native_jsvDrawFrame(JNIEnv *env, jobject thiz) {
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);

    int ret = ijkmp_jsv_draw_frame(mp);
