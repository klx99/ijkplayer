...
// JsView Added >>>
void ijkmp_jsv_set_video_sync_callback(IjkMediaPlayer *mp, bool (*callback)(void *opaque), void *opaque)
{
    if (!mp)
        return;
    ...
    if (mp && mp->ffplayer && mp->ffplayer->pipeline) {
        ffpipeline_jsv_set_video_sync_callback(mp->ffplayer->pipeline, callback, opaque);
    }
